...
%   functions.  The normalization factor sqrt((2-delta(m,0))*(n+1)), with
%   delta(m,0) the Kronecker delta, is chosen so that the integral of (r *
%   [Znm(r,theta)]^2) over the unit circle (from r=0 to r=1, and theta=0 to
%   theta=2*pi) is pi, or equivalently the Var(Z)_{unit circle} = 1.  For
%   the non-normalized polynomials, max(Znm(r=1,theta))=1 for all [n,m].
%
...
%
%                                              updated by Hao,Xiang (delete /sqrt(pi))
...
%   updated by Xiang Hao  Jan 10, 2015: change integral on unit circle to pi
%   updated by Xin Liu  Apr 13, 2023: add note Var(Z) = 1;
